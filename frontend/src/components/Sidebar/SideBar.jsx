  ...
  return (
    <div className="relative">
      ...
      >
        <Link
          to="/"
          className="flex items-center py-4 lg:mt-0 mt-10 text-2xl font-bold"
        >
          <IoLogoAndroid className="w-12 h-12 mr-2" />
          <span className="md:inline">App Name</span>
        </Link>
        <nav className="flex-1">
          <ul className="space-y-2">
            ...
            <li>
              <Link
                to={`/profile/${user?.user?._id}`} // Adjust this path according to how you handle profile routes
                className="flex items-center py-2 px-4 rounded-md transition-colors hover:bg-gray-300"
              >
